 ...
 */
/**
 * 新增297-303行代码使quartz支持fiexed-delay
...
public class QuartzSchedulerThread extends Thread {
	 ...
	 *            the time (in millis) when the newly scheduled trigger will fire. If this method is being called do to some other even (rather than scheduling a trigger), the caller should pass zero
	 *            (0).
	 */
			...
			try {
				...
				if (availThreadCount > 0) { // will always be true, due to semantics of blockForAvailableThreads...
					...
					if (triggers != null && !triggers.isEmpty()) {
						...
						/**
						 * 新增下面7行代码支持quartz的fiexed-delay
						 */
						if (now - triggerTime > 0) {
							try {
								Thread.sleep(triggerTime - triggers.get(0).getPreviousFireTime().getTime());
